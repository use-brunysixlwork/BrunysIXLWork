function createUnityInstance(e, t, r) {
	...
	function dowloadPart(e, part){
		var t = h.cacheControl(h[e]+".00"+part),
	...
	async function combinePart(e){
		var allFile = new Uint8Array(h.size);
		for(i = 0; i < h.totalFile; i++){
			var tmp = await dowloadPart(e, i);
			console.log("--fx--",tmp);
			var size = h.split_size;
	...
	function u(e) {
		l(e);
        console.log("--fx--",e,h.totalFile);
		if(e == "dataUrl"  && h.totalFile){
			console.log("--fx--",h.size);
			
			return combinePart(e).then((t) => {
				console.log(t);
				return d(t, h[e]);
			})
	...
	for (var b in t) h[b] = t[b];
	h.streamingAssetsUrl = new URL(h.streamingAssetsUrl, document.URL).href;
	var m = h.disabledCanvasEvents.slice();
	m.forEach(function(t) {
		e.addEventListener(t, i)
	}), window.addEventListener("error", o), window.addEventListener("unhandledrejection", o), h.deinitializers.push(function() {
		h.disableAccessToMediaDevices(), m.forEach(function(t) {
			e.removeEventListener(t, i)
		}), window.removeEventListener("error", o), window.removeEventListener("unhandledrejection", o);
		for (var t in h.intervals) window.clearInterval(t);
		h.intervals = {}
	}), h.QuitCleanup = function() {
		for (var e = 0; e < h.deinitializers.length; e++) h.deinitializers[e]();
		h.deinitializers = [], "function" == typeof h.onQuit && h.onQuit()
	};
	var g = "",
		p = "";
	...
	h.SystemInfo = function() {
		...
		return s(e, "", 0), !0
	}, Error.stackTraceLimit = Math.max(Error.stackTraceLimit || 0, 50), h.fetchWithProgress = function() {
		function e(e, t) {
			if (!t) return 0;
				...
				var t = {
						...
						function f(e, t) {
							...
							e: for (;;) switch (r.mode) {
								case N:
									...
									if (2 & r.wrap && 35615 === f) {
										r.check = 0, Be[0] = 255 & f, Be[1] = f >>> 8 & 255, r.check = y(r.check, Be, 2, 0), f = 0, h = 0, r.mode = z;
										break
									}
					...
					"zlib/inftrees.js": function(e, t, r) {
						...
						t.exports = function(e, t, r, b, m, g, p, w) {
							...
							if (e === s ? (F = M = p, S = 19) : e === l ? (F = u, Z -= 257, M = c, W -= 257, S = 256) : (F = f, M = h, S = -1), z = 0, R = 0, T = L, _ = g, A = I, P = 0, k = -1, N = 1 << I, x = N - 1, e === l && N > a || e === d && N > i) return 1;
							for (;;) {
								E = T - P, p[R] < S ? (C = 0, B = p[R]) : p[R] > S ? (C = M[W + p[R]], B = F[Z + p[R]]) : (C = 96, B = 0), v = 1 << T - P, y = 1 << A, L = y;
								do y -= v, m[_ + (z >> P) + y] = E << 24 | C << 16 | B | 0; while (0 !== y);
								for (v = 1 << T - 1; z & v;) v >>= 1;
	...
	return new Promise(function(e, t) {
		h.SystemInfo.hasWebGL ? h.SystemInfo.hasWasm ? (1 == h.SystemInfo.hasWebGL && h.print('Warning: Your browser does not support "WebGL 2" Graphics API, switching to "WebGL 1"'), h.startupErrorHandler = t, r(0), h.postRun.push(function() {
			r(1), delete h.startupErrorHandler, e(w)
